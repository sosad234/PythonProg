...
## Задание
1. Реализовать парсер с использованием Selenium для сбора данных с веб-страницы. Это может быть:
    - онлайн-каталог
    - интернет-магазин
    - энциклопедия и т.д.
...
3. Написать запросы для выборки данных из БД с использованием PyPika Queru Builder
    - 2 запроса с JOIN
    - 3 запроса с расчетом статистики/группировкой/агрегирующими функциями
4. Оформить отчет `README.md`

## Ход работы

Для парсинга была выбрана [страница из энциклопедии Wikipedia](https://en.wikipedia.org/wiki/List_of_video_games_considered_the_best).
...
cursor.execute(str(query))
print("Группировка игр по названиям платформ:")
for row in cursor.fetchall():
    print(row)
